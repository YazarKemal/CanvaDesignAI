...
BRAND_KIT_ASSETS: dict[str, dict[str, str]] = {
    "logo_emblem": {
        ...
        "label": "🪧 Logo & Emblem",
        "concept_override": "brand identity presentation layout",
        "layout_directive": (
            "LOGO & EMBLEM — BEHANCE BRANDING PRESENTATION LAYOUT (strict):\n"
            "- This is a BEHANCE PORTFOLIO BRAND IDENTITY SLIDE, not a simple logo.\n"
            "- Layer 1 (raster_background): 'clean minimalist stationery & brand "
            "mockup layout' — a top-down flat-lay composition of overlapping branded "
            "items: a letterhead sheet, a kraft envelope, and 2-3 branded business "
            "cards arranged at slight angles. One item should carry the brand emblem "
            "prominently. The background beneath the items is a deep editorial matte "
            "colour — deep cobalt blue (#1A2744) or midnight charcoal (#1C1C1E). "
            "The items sit on this dark surface with soft drop shadows. NEVER 'flat "
            "vector illustration', NEVER isolated icon on white. negative_prompt "
            "MUST include 'no gibberish text, no embedded letters, no fake words, "
            "no stock vector clip-art, no isolated icon on white, no plain logo'.\n"
            "- 50% of the canvas MUST be the deep matte background — the stationery "
            "items occupy at most 50% of the frame, with generous breathing room.\n"
            "- Layer 2 (vector_elements): thin editorial border frame (1 px, muted "
            "brass or warm neutral) inset 48 px from all edges. Optional: corner "
            "brackets at the frame corners. No pill_button, no badge, no CTA.\n"
            "- Layer 3 (native_typography): NO centered text block. Use a 4-CORNER "
            "MICRO-METADATA GRID distributed across the canvas corners in 8-9 pt "
            "Inter Regular or the brand's body font, in a muted warm tone:\n"
            "  · Top-Left: 'BRAND NAME · @2026'\n"
            "  · Top-Right: 'IDENTITY SYSTEM · VOL.01'\n"
            "  · Bottom-Left: 'CRAFTED IN TURKEY'\n"
            "  · Bottom-Right: 'BRAND SYSTEM LOCKUP'\n"
            "- The headline (brand name, 64-72 pt bold display) is the ONLY large "
            "element — positioned centered-left or centered-right, overlapping "
            "the stationery items slightly. This creates a radical point-size "
            "contrast: 72 pt headline vs 8 pt corner labels (9:1 ratio).\n"
            "- Subtext: STRICTLY 'EST. 2026' only (max 2 words).\n"
            "- Palette background: deep cobalt #1A2744 or midnight #1C1C1E. "
            "Text/accents: brand-approved colors for warmth against the dark base."
        ),
    },
    "opening_poster": {
        ...
        "layout_directive": (
            "GRAND OPENING POSTER — BEHANCE EDITORIAL LAYOUT (strict):\n"
            ...
            "50% of the canvas MUST be low-detail negative space.\n"
            "- Layer 2 (vector_elements): thin editorial border frame (1 px, muted "
            "brass) inset 48 px. Three info blocks in a clean asymmetric grid:\n"
            "  1. Hero number/date: '15.08' in giant 96 pt display weight.\n"
            "  2. 'GRAND OPENING' label in the brand's headline font.\n"
            "  3. Location: '123 Brew Lane, Portland' in small refined body font.\n"
            "- Layer 3 (native_typography): headline is the event title (64-72 pt); "
            "subtext is a one-line tagline (max 6 words). alignment_zone: top-left "
            "or center-left, never centered.\n"
            "- 4-CORNER MICRO-METADATA (8 pt, muted tone):\n"
            "  · Top-Left: brand name\n"
            "  · Top-Right: 'EDITORIAL · EVENT'\n"
            "  · Bottom-Left: date in full: 'SATURDAY / 15.08.2026'\n"
            "  · Bottom-Right: '@BRANDNAME'"
        ),
    },
    "menu_list": {
        ...
        "layout_directive": (
            "MENU & PRODUCT LIST — BEHANCE CATALOGUE LAYOUT (strict):\n"
            ...
            "bottom 15% must be completely empty negative space. 50% of the total "
            "canvas must remain strictly empty. "
            ...
            "(thin horizontal rules at 0.5 px in a muted brand color). Thin border "
            "frame (1 px) inset 36 px. No pill_button, no badge.\n"
            "- Layer 3 (native_typography): headline is the menu title (64-72 pt, "
            "bold display). Subtext is a brief descriptor (max 4 words).\n"
            "- 4-CORNER MICRO-METADATA (8 pt, muted tone):\n"
            "  · Top-Left: brand name\n"
            "  · Top-Right: 'MENU · VOL.01'\n"
            "  · Bottom-Left: 'PRICES IN USD · SUBJECT TO CHANGE'\n"
            "  · Bottom-Right: '@BRANDNAME · EST. 2026'\n"
            "- direct_action_tip MUST describe a PRICED MENU LAYOUT with category "
            "headers + dot-leader price alignment."
        ),
    },
    "packaging_merch": {
        ...
        "layout_directive": (
            "PACKAGING & MERCH — BEHANCE PRODUCT MOCKUP LAYOUT (strict):\n"
            "- This is a BOX DIE-CUT PACKAGING SLEEVE / REPEATING BRAND PATTERN "
            "GRID template — NOT a poster, NOT a social media graphic.\n"
            ...
            "packaging sleeve template'. 50% of the canvas must remain as the clean "
            "repeating pattern with no additional elements. "
            "NEVER 'flat vector illustration', NEVER 'editorial photography', NEVER "
            "scene descriptions. negative_prompt MUST include 'no gibberish text, "
            "no embedded letters, no fake words, no stock vector clip-art, no poster "
            "layout, no social media template'.\n"
            "- Layer 2 (vector_elements): a centered die-cut outline frame OR corner "
            "brackets marking the label zone. A subtle repeating geometric motif "
            "in the background. No CTA, no badge, no button.\n"
            "- Layer 3 (native_typography): headline is the brand name (64-72 pt, "
            "centered). Subtext STRICTLY limited to 2-3 words. alignment_zone: center.\n"
            "- 4-CORNER MICRO-METADATA (8 pt, muted tone):\n"
            "  · Top-Left: 'PACKAGING SYSTEM'\n"
            "  · Top-Right: 'DIE-CUT · KRAFT STOCK'\n"
            "  · Bottom-Left: brand name + '@2026'\n"
            "  · Bottom-Right: 'MADE IN TURKEY'\n"
            "- The result must look like a physical product label template on a "
            "Behance packaging design portfolio page."
        ),
    },
}
