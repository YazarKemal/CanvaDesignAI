...
function jsonRequest(body: unknown) {
  return new Request("http://localhost/api/mockup/refine", {
    method: "POST",
...
describe("POST /api/mockup/refine", () => {
  afterEach(() => vi.restoreAllMocks());

  it("forwards the JSON payload unchanged", async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      new Response(JSON.stringify({ environment: "studio" }), { status: 200 }),
    );
    ...
    expect(url).toBe("http://127.0.0.1:8002/api/mockup/workflow/refine");
  ...
  it("returns 400 for invalid JSON", async () => {
    const req = new Request("http://localhost/api/mockup/refine", {
      method: "POST",
      body: "not-json",
    });
